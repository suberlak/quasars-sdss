...
modified  qso_CRTS_SDSS_matched_plotting.py

meant to plot the log(sigma) vs log(tau)  for Chelsea S82 results
matched to Javelin fits of SDSS r-band  data,  from  chelsea_results_SDSS_load.py
...
dir_in = 'QSO_SDSS_analysis/'  
dir_out = 'QSO_SDSS_analysis/'


results_file =  'javelin_SDSS_chelsea_comparison_r_band.txt'
...
for i in range(len(good_LC)):
    good_LC_cut = np.append(good_LC_cut, good_LC[i][2:-4])
...
for i in range(len(qso_name)):
    ...
    good_LC_mask[i] =  qso_name[i][:-14] in good_LC_cut     
...
def histogram2D(x_arr, y_arr, number, percent, xlim, ylim, title, dir_out):
    ...
    if title == 'jav' :
        ...
        title_hist = 'SDSS Javelin results, '+ str(number) + ', i.e.  ' + str(percent)[:5]+ '% points'
        fname = dir_out + 'SDSS_Javelin_matched_sigma_hat_tau.png'
    if title == 'ss' : 
        plt.xlabel(x_label_ch,fontsize=font+5)
        plt.ylabel(x_label_jav,fontsize=font+5)
        title_hist = 'S82 SDSS Chelsea vs JAVELIN r-band, '+str(number) + ', i.e.  ' + str(percent)[:5]+ '% points'
        fname = dir_out + 'SDSS_SDSS_matched_sigma_hat_sigma_hat.png'
    if title == 'tt' :
        plt.xlabel(y_label_ch, fontsize=font+5)
        plt.ylabel(y_label_jav,fontsize=font+5)
        title_hist = 'S82 SDSS Chelsea vs JAVELIN r-band, '+str(number) + ', i.e.  ' + str(percent)[:5]+ '% points'
        fname = dir_out + 'SDSS_SDSS_matched_tau_tau.png'  
...
print '\nMedian and RMs from IQR (75-25%) range for Javelin SDSS sigma hat is ', median_and_rms(y_arr)