...
fig_name=['qso_drw_S_M_Chelsea_results/drw_'+length+'_prior-'+prior+'_err','_log_sigma_ratio_vs_log_tau_ratio.png']

print 'Using Chelsea results from', results_ch,' and Javelin results from ', results_jav, 'n_errors=,', n_errors
...
for k in range(1,n_errors+1):   #n_errors+1
    ...
    ratio_sigma = np.log10( sigma_m_jav / np.power(10,log_sigma_ch) )
    ratio_tau   = np.log10( tau_m_jav / np.power(10,log_tau_ch) )
    ...
    x , y, num, percent = load_x_y(ratio_sigma ,ratio_tau,jav_indices[k-1], k, x_lim, y_lim)
